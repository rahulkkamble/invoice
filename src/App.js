...
/* Map global window.GlobalPractitioner FHIR object to { id, name, license } with safe fallbacks */
function resolveGlobalPractitioner() {
  const gp =
    (typeof window !== "undefined" &&
      (window.GlobalPractitioner || window.GlobalPractitionerFHIR)) ||
    null;

  if (!gp) return { ...DEFAULT_PRACTITIONER };

  // If already flattened
  if (gp && typeof gp === "object" && typeof gp.name === "string") {
    return {
      id: gp.id || DEFAULT_PRACTITIONER.id,
      name: gp.name || DEFAULT_PRACTITIONER.name,
      license: gp.license || DEFAULT_PRACTITIONER.license,
    };
  }

  // FHIR Practitioner shape
  const id = gp?.id || DEFAULT_PRACTITIONER.id;
  const name =
    (Array.isArray(gp?.name) && gp?.name?.[0]?.text) ||
    (typeof gp?.name === "string" ? gp?.name : "") ||
    DEFAULT_PRACTITIONER.name;
  const license =
    (Array.isArray(gp?.identifier) && gp?.identifier?.[0]?.value) ||
    (typeof gp?.license === "string" ? gp?.license : "") ||
    DEFAULT_PRACTITIONER.license;
...
export default function App() {
  ...
  const [lineItems, setLineItems] = useState([
    { id: uuidv4(), description: "Consultation", quantity: 1, unit: "each", unitPrice: 500, taxPercent: 0 },
  ]);
  ...
  /* Re-resolve practitioner if global attaches late (no UI logic change) */
  useEffect(() => {
    const resolved = resolveGlobalPractitioner();
    setPractitioner((prev) =>
      prev.name !== resolved.name || prev.license !== resolved.license || prev.id !== resolved.id
        ? resolved
        : prev
    );
  }, []);
  ...
  function addLineItem() {
    setLineItems((s) => [
      ...s,
      { id: uuidv4(), description: "", quantity: 1, unit: "each", unitPrice: 0, taxPercent: 0 },
    ]);
  }
  ...
  function handleFileChange(ev) {
    ...
    files.forEach((f) => {
      ...
      reader.onload = (e) => {
        ...
        setAttachments((s) => [
          ...s,
          { id: uuidv4(), name: f.name, contentType: f.type || "application/octet-stream", data: base64 },
        ]);
      };
  ...
  /* ---------- build FHIR bundle (logic preserved) ---------- */
  function buildBundle() {
    if (selectedPatientIdx === null) {
      alert("Please select a patient and ABHA address.");
      return null;
    }
    ...
    const patientId =
      selPatient.user_ref_id && typeof selPatient.user_ref_id === "string"
        ? selPatient.user_ref_id.toLowerCase()
        : uuidv4();
    const compId = uuidv4();
      ...
      const components = [
        {
          ...
          code: {
            coding: [
              {
                system: "http://nrces.in/CodeSystem/price-component",
                code: "base-price",
                display: "Base price",
              },
            ],
          ...
          amount: { value: Number(base.toFixed(2)), currency: "INR" },
        },
      ];
      if (tax > 0) {
        components.push({
          ...
          code: {
            coding: [
              { system: "http://nrces.in/CodeSystem/price-component", code: "gst", display: "GST" },
            ],
            text: "GST",
          },
          amount: { value: Number(tax.toFixed(2)), currency: "INR" },
        });
      }
      const textDesc = li.description
        ? `${li.description} (${qty} ${li.unit || "unit"})`
        : `${qty} x ${li.unit || "unit"}`;
      return {
        sequence: idx + 1,
        chargeItemCodeableConcept: {
          coding: [
            {
              system: "http://nrces.in/CodeSystem/invoice-item",
              code: `item-${idx + 1}`,
              display: li.description || `Item ${idx + 1}`,
            },
          ],
          text: textDesc,
        },
    ...
    const computedNet = processedLineItems.reduce(
      (s, it) => s + ((it.priceComponent || []).find((c) => c.type === "base")?.amount?.value || 0),
      0
    );
    const computedTax = processedLineItems.reduce(
      (s, it) => s + ((it.priceComponent || []).find((c) => c.type === "tax")?.amount?.value || 0),
      0
    );
    const computedGross = computedNet + computedTax;

    const finalNet =
      totalNet && !isNaN(Number(totalNet)) ? Number(totalNet) : Number(computedNet.toFixed(2));
    const finalTax =
      totalTax && !isNaN(Number(totalTax)) ? Number(totalTax) : Number(computedTax.toFixed(2));
    const finalGross =
      totalGross && !isNaN(Number(totalGross))
        ? Number(totalGross)
    ...
    const compositionResource = {
      ...
      type: {
        coding: [
          { system: "http://nrces.in/CodeSystem/document-type", code: "INVR", display: "Invoice Record" },
        ],
        text: "Invoice Record",
      },
    ...
    // Axios POST (added) with original patient id; log id and errors to console
    const originalPatientId = String(selPatient?.id || selPatient?.user_ref_id || "");
    console.log("Submitting for patient:", originalPatientId);
    axios
      .post("https://uat.discharge.org.in/api/v5/fhir-bundle", {
        bundle,
  ...
  return (
    <div className="container my-4">
      ...
      <p className="text-muted">
        Select patient → ABHA → fill invoice, add items/tax, attach files, generate FHIR Bundle.
      </p>
        ...
        <div className="card-body">
          ...
          <div className="row g-2">
            ...
            <div className="col-md-6">
              ...
              <select
                className="form-select"
                disabled={selectedPatientIdx === null}
                value={selectedAbha}
                onChange={(e) => setSelectedAbha(e.target.value)}
              >
                <option value="">-- select ABHA --</option>
                {selectedPatientIdx !== null &&
                  getAbhaAddressesForPatient(patients[selectedPatientIdx]).map((a, idx) => (
          ...
          {selectedPatientIdx !== null && (
            <div className="mt-2">
              <small className="text-muted">
                Selected: {patients[selectedPatientIdx].name} • {patients[selectedPatientIdx].gender} • DOB:{" "}
                {patients[selectedPatientIdx].dob}
              </small>
            </div>
          )}
        </div>
        ...
        <div className="card-body">
          ...
          <div className="row g-2">
            <div className="col-md-5">
              ...
              <input className="form-control" value={practitioner.name} readOnly />
            </div>
            <div className="col-md-4">
              <label className="form-label">License *</label>
              <input className="form-control" value={practitioner.license} readOnly />
            </div>
          ...
          <div className="row g-2">
            ...
            <div className="col-md-3">
              ...
              <input
                className="form-control"
                value={organization.gstin}
                onChange={(e) => setOrganization((o) => ({ ...o, gstin: e.target.value }))}
              />
            </div>
            <div className="col-md-3">
            ...
            <div className="col-md-12 mt-2">
              ...
              <input
                ...
                onChange={(e) => setOrganization((o) => ({ ...o, address: e.target.value }))}
              />
            </div>
          </div>
        </div>
        ...
        <div className="card-body">
          ...
          <div className="row g-2 mt-2">
            ...
            <div className="col-md-3">
              ...
              <input className="form-control" placeholder="Optional note" onChange={() => {}} />
            </div>
          </div>
            ...
            <div className="row g-2 align-items-end mb-2" key={li.id}>
              ...
              <div className="col-md-1">
                ...
                <input
                  ...
                  className="form-control"
                  value={li.taxPercent}
                  onChange={(e) => updateLineItem(li.id, { taxPercent: Number(e.target.value) })}
                />
              </div>
              <div className="col-md-12 mt-1 d-flex gap-2">
                <button
                ...
                {li === lineItems[lineItems.length - 1] && (
          ...
          <div className="row mt-3">
            ...
            <div className="col-md-3">
              <label className="form-label">Total Tax (INR)</label>
              <input
                className="form-control"
                value={totalTax}
              ...
              {attachments.map((a) => (
                ...
                >
                  <div>
                    ...
                    <small className="text-muted">({a.contentType})</small>
                  </div>
                  <div>
                    <button className="btn btn-sm btn-danger" onClick={() => removeAttachment(a.id)}>
                      Remove
                    </button>
                  </div>
                </div>
          ...
          <div className="mt-3">
            <button className="btn btn-primary me-2" onClick={() => buildBundle()}>
              Generate Bundle
            </button>
            <button
              className="btn btn-outline-secondary"
              onClick={() => downloadBundle()}
              disabled={!generatedBundleJson}
            >
              Download Bundle
            </button>
          </div>
      ...
      <div className="card mb-5">
        <div className="card-body">
          ...
          {!generatedBundleJson ? (
            <div className="text-muted">No bundle generated yet.</div>
          ) : (
            <pre style={{ maxHeight: 420, overflow: "auto", background: "#f8f9fa", padding: 12 }}>
              {generatedBundleJson}
            </pre>
          )}
        </div>
      ...
      <footer className="text-muted small">
        Notes: This version fixes the validator issues you reported (author/attester references,
        qualification display, invalid invoice fields). You may still see warnings for NDHM code
        systems (expected). If you want me to swap some NDHM code systems to HL7 to reduce warnings,
